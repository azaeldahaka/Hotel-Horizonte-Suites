      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            ...
            <p className="text-slate-600">¿Listo para tu próxima estancia en Horizonte Suites?</p>
          </div>
          <button
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-all border-l-4 border-l-teal-500">
      <div className="flex-1">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1 ${getEstadoColor(reserva.estado)} md:hidden`}>{getEstadoIcon(reserva.estado)}<span className="capitalize">{reserva.estado}</span></span>
        </div>
        <p className="text-sm text-slate-500 mb-1">{new Date(reserva.fecha_entrada).toLocaleDateString('es-ES')} - {new Date(reserva.fecha_salida).toLocaleDateString('es-ES')}</p>
        <div className="flex items-center gap-4 text-sm"><p className="flex items-center gap-1 text-slate-600"><Users className="h-4 w-4 text-teal-500"/> {reserva.num_huespedes} pax</p><p className="font-semibold text-amber-600">Total: ${reserva.total.toLocaleString('es-ES')}</p></div>
      </div>
      <div className="flex flex-col items-end gap-2 w-full md:w-auto">
        ...
        {tieneCambioPendiente ? ( <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 text-amber-700 rounded-lg border border-amber-200 w-full md:w-auto justify-center"><RefreshCw className="h-4 w-4 animate-spin-slow" /><span className="font-medium text-sm">Solicitud Pendiente</span></div> ) : cambioAprobado ? ( <div className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-lg border border-green-200 w-full md:w-auto justify-center"><CheckCircle className="h-4 w-4" /><span className="font-medium text-sm">Cambio Realizado</span></div> ) : ( reserva.estado === 'activa' && ( <button onClick={onSolicitarCambio} className="w-full md:w-auto px-4 py-2 bg-white hover:bg-slate-50 text-slate-700 rounded-lg font-medium flex items-center justify-center gap-2 transition-colors border border-slate-300 shadow-sm hover:shadow"><Edit className="h-4 w-4" />Solicitar Cambio</button> ) )}
      </div>
    </div>
  )
}
...
const ModalSolicitarCambio = ({ reserva, habitaciones, onClose, onSuccess, user_id }: any) => {
  ...
  const habitacionesFiltradas = habitaciones.filter((h:any) => h.tipo === selectedTipo);

  useEffect(() => {
    try { const { fecha_entrada, fecha_salida, habitacion_id } = formData; const habInfo = habitaciones.find((h:any) => h.id === habitacion_id); if (fecha_entrada && fecha_salida && habInfo) { const dias = Math.ceil((new Date(fecha_salida + 'T00:00:00').getTime() - new Date(fecha_entrada + 'T00:00:00').getTime())/(1000*60*60*24)); if(dias>0){const tot=dias*habInfo.precio_noche; setNuevoTotal(tot); setDiferencia(tot-reserva.total);} } } catch (e) { console.error(e); }
  ...
  const handleSubmit = async (e: any) => { e.preventDefault(); setLoading(true); setError(''); try { if(new Date(formData.fecha_salida) <= new Date(formData.fecha_entrada)) throw new Error("Fechas inválidas"); const { data, error: funcError } = await supabase.functions.invoke('check-room-availability', { body: { habitacion_id: formData.habitacion_id, fecha_entrada: formData.fecha_entrada, fecha_salida: formData.fecha_salida, reserva_id_excluir: reserva.id } }); if (funcError || !data?.data?.available) throw new Error('No disponible'); const habOriginal = habitaciones.find((h:any) => h.id === reserva.habitacion_id); const habNueva = habitaciones.find((h:any) => h.id === formData.habitacion_id); const mensaje = JSON.stringify({ type: 'SOLICITUD_CAMBIO_ESTRUCTURADA', reservaId: reserva.id, original: { habNumero: habOriginal?.numero, habTipo: habOriginal?.tipo, entrada: reserva.fecha_entrada, salida: reserva.fecha_salida, total: reserva.total }, nuevo: { habitacion_id: habNueva?.id, habNumero: habNueva?.numero, habTipo: habNueva?.tipo, entrada: formData.fecha_entrada, salida: formData.fecha_salida, total: nuevoTotal, diferencia: diferencia } }); await supabase.from('consultas').insert([{ usuario_id: user_id, asunto: `Solicitud Cambio - Reserva #${reserva.id.slice(0,8)}`, mensaje: mensaje, estado: 'pendiente', fecha_consulta: new Date().toISOString() }]); setSuccess(true); } catch (err: any) { setError(err.message); } finally { setLoading(false); } };

  if (success) return <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-xl p-8 max-w-md w-full text-center"><CheckCircle className="h-16 w-16 text-green-500 mx-auto mb-4" /><h2 className="text-2xl font-bold text-slate-900 mb-2">¡Solicitud Enviada!</h2><button onClick={onSuccess} className="px-6 py-2 bg-teal-600 text-white rounded-lg font-medium">Entendido</button></div></div>;
  return <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-xl p-6 max-w-xl w-full"><h2 className="text-xl font-bold mb-6">Solicitar Cambio</h2>{error && <p className="text-red-500">{error}</p>}<form onSubmit={handleSubmit} className="space-y-4"><div className="grid grid-cols-2 gap-4"><input type="date" name="fecha_entrada" value={new Date(formData.fecha_entrada).toISOString().split('T')[0]} onChange={handleChange} className="border p-2 rounded"/><input type="date" name="fecha_salida" value={new Date(formData.fecha_salida).toISOString().split('T')[0]} onChange={handleChange} className="border p-2 rounded"/></div><select value={selectedTipo} onChange={handleTipoChange} className="w-full border p-2 rounded">{[...new Set(habitaciones.map((h:any)=>h.tipo))].map((t:any)=><option key={t} value={t}>{t}</option>)}</select><div className="bg-slate-50 p-4 rounded"><p>Diferencia: {diferencia}</p></div><div className="flex gap-3"><button disabled={loading} className="flex-1 bg-teal-600 text-white py-2 rounded">Enviar</button><button type="button" onClick={onClose} className="flex-1 bg-slate-200 py-2 rounded">Cancelar</button></div></form></div></div>
}

const ModalDetalleServicio = ({ servicio, onClose }: any) => { return <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}><div className="bg-white p-8 rounded-xl max-w-md relative" onClick={e=>e.stopPropagation()}><button onClick={onClose} className="absolute top-4 right-4">X</button><h2 className="text-2xl font-bold mb-4">{servicio.nombre}</h2><p>{servicio.descripcion}</p><p className="mt-4 font-bold text-teal-600">${servicio.precio}</p></div></div> }
const HabitacionCard = ({ habitacion, navigate }: any) => { return <div className="bg-white rounded-xl overflow-hidden shadow-lg"><div className="relative h-48"><img src={habitacion.imagen_url || '/images/rooms/room-1.jpg'} className="w-full h-full object-cover"/><div className="absolute top-4 right-4 bg-white px-3 py-1 rounded-full font-bold">{habitacion.numero}</div></div><div className="p-6"><h3 className="text-xl font-bold">{habitacion.tipo}</h3><p className="text-slate-600 text-sm">{habitacion.descripcion}</p><div className="flex justify-between items-center mt-4"><p className="text-2xl font-bold text-amber-600">${habitacion.precio_noche}</p><button onClick={()=>navigate(`/usuario/reservar/${habitacion.id}`)} className="px-4 py-2 bg-teal-600 text-white rounded transition">Reservar</button></div></div></div> }
const ServicioCard = ({ servicio, onClick }: any) => { return <div onClick={onClick} className="bg-white p-6 rounded-xl shadow-sm border hover:shadow-lg cursor-pointer"><h3 className="font-bold">{servicio.nombre}</h3><p className="text-sm text-slate-500">{servicio.descripcion}</p><p className="text-teal-600 font-bold mt-2">${servicio.precio}</p></div> }