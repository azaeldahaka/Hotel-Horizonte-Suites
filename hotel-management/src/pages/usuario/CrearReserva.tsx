...
// Función auxiliar para fechas locales (Evita el bug de zona horaria)
...
export const CrearReserva = () => {
  ...
  // Fecha mínima de salida (+1 día desde la entrada o desde hoy)
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mb-8 text-center md:text-left">
          <h1 className="text-3xl font-serif font-bold text-slate-900 mb-2">
            Reservar Habitación <span className="text-teal-600">{habitacion.numero}</span>
          </h1>
        ...
        <div className="flex items-center justify-center mb-8">
          ...
          <div className="w-12 md:w-24 h-1 bg-slate-300 mx-2 md:mx-4"></div>
        ...
        {paso === 'fechas' && (
          <div className="bg-white rounded-xl p-8 shadow-lg">
            <h2 className="text-2xl font-bold mb-6 font-serif text-slate-900">Selecciona las Fechas</h2>
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  <Calendar className="inline h-4 w-4 mr-2 text-teal-600" />
                  Fecha de Entrada
                </label>
                <input
                  type="date"
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-slate-700 mb-2">
                <Users className="inline h-4 w-4 mr-2 text-teal-600" />
                Número de Huéspedes
              </label>
              <input
                type="number"
        ...
        {paso === 'pago' && (
          <div className="bg-white rounded-xl p-8 shadow-lg">
            <h2 className="text-2xl font-bold mb-6 font-serif text-slate-900">Información de Pago</h2>
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                {(['tarjeta', 'efectivo', 'transferencia'] as const).map((metodo) => (
                  <button key={metodo} onClick={() => setMetodoPago(metodo)} className={`p-4 rounded-lg border-2 transition-all ${metodoPago === metodo ? 'border-teal-600 bg-teal-50 text-teal-700' : 'border-slate-300 hover:border-slate-400'}`}>
            ...
            {metodoPago === 'tarjeta' && (
              <div className="space-y-4 mb-6">
                <div><label className="block text-sm font-medium text-slate-700 mb-2">Número de Tarjeta</label><input type="text" value={datosTarjeta.numero} onChange={(e) => { const v = e.target.value.replace(/\s/g, '').replace(/\D/g, ''); setDatosTarjeta({ ...datosTarjeta, numero: v.match(/.{1,4}/g)?.join(' ') || v })}} placeholder="1234 5678 9012 3456" maxLength={19} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" /></div>
                <div><label className="block text-sm font-medium text-slate-700 mb-2">Titular</label><input type="text" value={datosTarjeta.titular} onChange={(e) => setDatosTarjeta({ ...datosTarjeta, titular: e.target.value })} placeholder="Juan Pérez" className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-slate-700 mb-2">Vencimiento</label><input type="text" value={datosTarjeta.expiracion} onChange={(e) => { let v = e.target.value.replace(/\D/g, ''); if (v.length >= 2) v = v.slice(0, 2) + '/' + v.slice(2, 4); setDatosTarjeta({ ...datosTarjeta, expiracion: v })}} placeholder="MM/AA" maxLength={5} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" /></div>
                  <div><label className="block text-sm font-medium text-slate-700 mb-2">CVV</label><input type="text" value={datosTarjeta.cvv} onChange={(e) => setDatosTarjeta({ ...datosTarjeta, cvv: e.target.value.replace(/\D/g, '') })} placeholder="123" maxLength={3} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none" /></div>
                </div>
              </div>
            )}
            
            {/* --- DATOS ACTUALIZADOS A HORIZONTE SUITES --- */}
            {metodoPago === 'efectivo' && (<div className="mb-6 p-4 bg-teal-50 rounded-lg border border-teal-200 text-teal-800">El pago se realizará en recepción al momento del Check-in.</div>)}
            {metodoPago === 'transferencia' && (
              <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200 text-blue-800">
                ...
                <p className="text-xs mt-2 opacity-80">Enviar comprobante por mail.</p>
              </div>
            )}

            <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
              <div className="flex justify-between items-center"><span className="text-slate-700 font-medium">Total a Pagar:</span><span className="text-2xl font-bold text-teal-600">${total.toLocaleString('es-ES')}</span></div>
            </div>
        ...
        {paso === 'confirmacion' && (
          <div className="bg-white rounded-xl p-8 shadow-lg text-center">
            ...
            <div className="bg-slate-50 rounded-lg p-6 mb-6 text-left border border-slate-200">
              <h3 className="font-bold mb-3 text-slate-900">Detalles de tu Reserva</h3>
            ...
            <button onClick={() => navigate('/usuario/dashboard')} className="px-8 py-3 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-lg transition-all">Volver al Dashboard</button>
          </div>
        )}
      </div>
    </div>
